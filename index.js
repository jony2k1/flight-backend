...
app.post("/generate-pkpass", async (req, res) => {
  try {
    ...
    console.log("[pkpass] Request received:", {
      from: flight.from,
      to: flight.to,
    ...
    // Coerce all field values to non-empty strings — Apple Wallet rejects passes
    // with null/undefined values and shows "type is missing" or fails silently.
    const s = (v, fallback = "—") => {
    ...
    const passData = {
      ...
      teamIdentifier: s(process.env.APPLE_TEAM_ID, "329H67W845"),
    ...
    try {
      ...
      console.log("[pkpass] Bundle generated successfully, size:", buffer.length, "bytes");
    } catch (passErr) {
      console.error("[pkpass] PKPass construction failed:", passErr?.message, passErr?.stack);
      return res.status(500).json({
        error: "Pass generation failed: " + (passErr?.message || "unknown"),
        stage: "construction",
      });
    }

    res.setHeader("Content-Type", "application/vnd.apple.pkpass");
    res.setHeader("Content-Disposition", `attachment; filename="flownto-${s(flight.from, "flight")}-${s(flight.to, "pass")}.pkpass"`);
    res.send(buffer);
  } catch (err) {
    console.error("[pkpass] outer error:", err?.message, err?.stack);
